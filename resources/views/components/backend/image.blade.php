...
<div class="single-input mb-3 {{$class}}">
    <label class="label-title mb-3" for="{{$name}}">{{__($title ?? '')}}</label>
    ...
    <div class="media-upload-btn-wrapper">
        <div class="img-wrap">
            ...
            @if (!empty($signature_img))
                <div class="rmv-span"><i class="{{isset($type) && $type === 'user' ? 'fas fa-times' : 'fas fa-times' }}"></i></div>
        ...
        <button type="button" class="btn btn-info media_upload_form_btn" data-btntitle="{{__('Select Image')}}" data-modaltitle="{{__('Upload Image')}}" data-imgid="{{$id ?? ''}}" data-bs-toggle="modal" data-bs-target="#media_upload_modal">
