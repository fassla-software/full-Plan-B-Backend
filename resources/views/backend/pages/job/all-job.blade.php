                                                ...
                                                <td>
                                                    @if (!empty($job['sub_category_image']))
                                                        <img width="60" height="60"
                                                            src="{{ $job['sub_category_image'] }}" alt="Job Image">
                                                    @else
                                                        <img width="60" height="60"
                                                            src="{{ asset('assets/uploads/no-image.png') }}"
                                                            alt="No Image">
                                                    @endif
                                                </td>
                                                    ...
                                                    <ul class="dropdown-menu status_dropdown__list">
                                                        <li class="status_dropdown__item">
                                                            <a href="{{ route('admin.job.details', $job['id']) }}"
                                                                class="btn dropdown-item status_dropdown__list__link">{{ __('View Details') }}</a>
                                                        </li>
                                                        <li class="status_dropdown__item">
                                                            <button
                                                                class="btn dropdown-item status_dropdown__list__link delete-job-button"
                                                                data-id="{{ $job['id'] }}">
                                                                {{ __('Delete Request') }}
                                                            </button>
                                                        </li>
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        error: function() {
                            Swal.fire('Error!', 'There was an error deleting the job.',
                            'error');
                        }
                    });
                }
