                    ...
                    <td>
                        ...
                        <ul class="dropdown-menu status_dropdown__list">
                            @can('user-details')
                                <li class="status_dropdown__item">
                                    <a class="btn dropdown-item status_dropdown__list__link user_details"
                                        ...
                                        data-user_id="{{ $user->id }}" data-type="{{ $user->user_type }}"
                                        ...
                                        data-city_id="{{ $user->city_id }}" data-total_jobs="{{ $user->total_jobs }}"
                                        data-total_equipment="{{ $user->total_equipment }}"
                                        data-commas="{{ $user->commas }}"
                                        data-remaining_commas="{{ $user->remaining_commas }}">
                            ...
                            @can('user-identity-details')
                                <li class="status_dropdown__item">
                                    <a class="btn dropdown-item status_dropdown__list__link user_identity_details"
                                        data-bs-toggle="modal" data-bs-target="#userIdentityModal"
                            ...
                            @can('user-account-status-change')
                                <li class="status_dropdown__item">
                                    @if ($user->is_suspend == 1)
                                        <x-status.table.status-change :class="'btn dropdown-item status_dropdown__list__link unsuspend_user_account'" :title="__('Unsuspend User')"
                                            :url="route('admin.account.unsuspend', $user->id)" />
                                    @else
                                        <x-status.table.status-change :class="'btn dropdown-item status_dropdown__list__link suspend_user_account'" :title="__('Suspend User')"
                                ...
                                @can('user-individual-commission-settings')
                                    <li class="status_dropdown__item">
                                        <a class="btn dropdown-item status_dropdown__list__link individual_commission_settings_modal"
                                            data-bs-toggle="modal" data-bs-target="#IndividualCommissionSettingsModal"
