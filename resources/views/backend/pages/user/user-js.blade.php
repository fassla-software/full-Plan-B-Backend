    ...
    (function($) {
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.user_details', function() {
                ...
                let remaining_commas = $(this).data('remaining_commas');
                ...
                $('#user_details .commas').text(commas);
                $('#user_details .remaining_commas').text(remaining_commas);

                country != '' ? $('#user_details .country').text(country) : $(
                    '#user_details .country').text('No country');
                state != '' ? $('#user_details .state').text(state) : $('#user_details .state')
                    .text('No state');
                city != '' ? $('#user_details .city').text(city) : $('#user_details .city').text(
                    'No city');
                ...
                $('#edit_user_details #edit_commas').val(commas);
                $('#edit_user_details #edit_remaining_commas').val(remaining_commas);
            });
            ...
            $(document).on('click', '.update_user_info', function() {
                ...
                let commas = $('#edit_user_details #edit_commas').val();
                let remaining_commas = $('#edit_user_details #edit_remaining_commas').val();

                if (first_name == '' || last_name == '' || username == '' || email == '' || phone ==
                    '' || country == '' || state == '' || city == '' || hourly_rate == '' ||
                    commas == '' ||
                    remaining_commas == '') {
            ...
            $(document).on('keyup', '#confirm_password', function() {
                ...
                if (password != confirm_password) {
                    $("#new_password_match").html(
                        "{{ __('New and confirm new password does not match !') }}").css(
                        "color", "red");
                } else {
                    $("#new_password_match").html("{{ __('Password match !') }}").css("color",
                        "green");
                }
            });
            ...
            $(document).on('click', '.change_user_password', function(e) {
                ...
                if (password != confirm_password) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function() {
                            $(".email_send_message").html(
                                "{{ __('Please wait while email is sending... !') }}"
                            ).css("color", "green");
                        },
                        success: function(res) {
                            if (res.status == 'ok') {
                                toastr_success_js(
                                    "{{ __('Password successfully change') }}")
                                $('#userPasswordModal').modal('hide');
                                $('#userPasswordModalForm')[0].reset();
                                $('.table_activation').load(location.href +
                                    ' .table_activation');
                                $("#new_password_match").html('');
                                $(".email_send_message").html('');
                            }
                            if (res.status == 'not_match') {
                                toastr_warning_js(
                                    "{{ __('Password and confirm password does not match!') }}"
                                )
                            }
                        }
                    });
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(res) {
                                toastr_success_js(
                                    "{{ __('Status successfully updated') }}"
                                )
                                $('.table_activation').load(location.href +
                                    ' .table_activation');
                            }
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(res) {
                                toastr_warning_js(
                                    "{{ __('Identity verify request successfully decline') }}"
                                )
                                $('.table_activation').load(location.href +
                                    ' .table_activation');
                            }
            ...
            function countries(page) {
                $.ajax({
                    url: "{{ route('admin.freelancer.paginate.data') . '?page=' }}" + page,
                    success: function(res) {
            ...
            $(document).on('keyup', '#string_search', function() {
                ...
                $.ajax({
                    url: "{{ route('admin.freelancer.search') }}",
                    method: 'GET',
                    data: {
