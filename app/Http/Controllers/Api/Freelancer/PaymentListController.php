...
class PaymentListController extends Controller
{
    public function gateway_list()
    {
        ...
        if (get_static_option('order_enable_disable_milestone_settings')){
            $data['milestone_settings'] = get_static_option('order_enable_disable_milestone_settings');
        }

        if(get_static_option('transaction_fee_type') && get_static_option('transaction_fee_charge')){
